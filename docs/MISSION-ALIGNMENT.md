...
| 31–36 Storage / obs / alerts | SQLite ledger events/snapshots + JSON logs (request/signal/trade/risk/ledger, redaction) + Prometheus from the paper ledger (equity/PnL/drawdown/win-rate after closed trades) + localhost `/` paper dashboard (SSE `/events` + `/api/state`) alongside `/metrics` `/health` `/ready` + alerts. Static GitHub Pages demo under `docs/pages/` (poll `state.json` / rotating mock snapshots; no live process). PAPER default. No LIVE. No invented venue PnL. |
...
| 43–47 Watchlist / filter / micro / audit | Watchlist + basic filter (book spread fallback). **Parte 45 L2 microstructure** on paper extras / feature snapshot / signal_quality. **Parte 46** every PAPER evaluate persists mission-shaped signal JSON (`fair_probability`, costs, half-life, HMS, opportunity, `TRADE\|SKIP`, `reason_codes`); nulls when the path has no value yet. **Parte 47** refused rows are first-class SQLite audits; skip tags reuse existing codes plus audit-only `IMPACT_EXHAUSTED` when `exhausted_*` is true, and `SPREAD_TOO_LARGE` when `spread_regime=wide`. Cold-path `hotflow skip-audit` / daily-review buckets by reason × spread_regime. `max_impact` / `min_top_depth` stay default-off. Risk VETO absolute. LIVE closed |
...
| 54–64 Preservation / repo / CI / phases | Docs + soaks + LIVE-gate freeze + `hotflow readiness` rollup (`live_ready=false`); LIVE still gated. **CI:** local `make ci` / `hotflow ci` / `scripts/ci_local.sh` is the source of truth. GitHub-hosted runners billing-locked (workflow kept, push triggers off). Free remote: GitLab.com `.gitlab-ci.yml` |
